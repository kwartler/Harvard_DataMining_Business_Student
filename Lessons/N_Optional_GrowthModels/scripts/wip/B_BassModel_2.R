...
fitBass   <- diffusion(hdTV[1:4, 3], type = "bass",w = bassParam)

# Predict
hdTVpreds <- predict(fitBass, h = nrow(hdTV)-4)

# Examine
hdTVpreds$frc

# Organize
finalDF <- data.frame(actual = hdTV$HDTV_shipments_M,
                      fit = c(hdTVpreds$fit[,1],hdTVpreds$frc[1:7,2]))

finalDF

plot(finalDF$actual, type ='l', col='darkgreen')
points(finalDF$fit, col='red')
