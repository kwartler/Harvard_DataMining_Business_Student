...
modelfit <- function(response, data, extras) {
  ...
  # Initialize some variables to control pre-processing
  ...
  #  Initialize some variables to control the fit

  # Choose the number of trees used to build the forest
  if (nrow(data) < 10000) {
    NTREES <- 350
  } else {
    NTREES <- 250
  ...
  # Choose MTRY
  MTRY <- min(max(floor(ncol(data2)/3),1), 15)

  # TODO:
  #   - ranger
  #   - no one-hot encoding (keep cats up to 12 as cats?)
